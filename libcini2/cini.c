...
void CINI_fill_rect_rgb(int x, int y, int w, int h, 
										Uint8 r, Uint8 g, Uint8 b)
...
void CINI_fill_rect(int x, int y, int w, int h, char* color)
{
   /* remplit avec "color" un rectangle de largeur w et hauteur h,
      dont le coin superieur gauche est en (x, y) */	CINI_color *c = CINI_parse_color(color);
	if (c != NULL)
...
void CINI_draw_char_table(int x, int y, char tab[], int size, char* bg_color, char* fg_color)
{
	...
	if (fgc != NULL) {
		...
		for(i = 0; i < size; i++) {
  		...
  		CINI_draw_line_rgb(x0+wTot+size*m, y, x0+wTot+size*m, y+hCase, fgc->r, fgc->g, fgc->b);
	}
	free(s);
}
  ...
  while (!quit && key == -1) {
    while(SDL_PollEvent(&event)) {
      switch(event.type) {
				...
				case SDL_KEYDOWN: key = CINI_key_pressed(&event);
			}
    }
  ...
  while (!quit && key == -1) {
    while(SDL_PollEvent(&event)) {
      switch(event.type) {
				...
				case SDL_QUIT: quit = true;
				// A key has been pressed.
				case SDL_KEYUP: key = CINI_key_pressed(&event);
			}
    }
...
#define COLOR_NAME_CMP(cs, name, code) if(strcmp(cs, name) == 0)	\
					  {free(cs); return parseHexColor(code); }
